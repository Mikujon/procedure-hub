...
const TRANSITIONS: Record<string, { to: ProcedureStatus; action: string; summary: string }[]> = {
  DRAFT: [{ to: "IN_REVIEW", action: "SUBMIT", summary: "Submitted for review" }],
  IN_REVIEW: [
    { to: "PUBLISHED", action: "PUBLISH", summary: "Approved and published" },
    { to: "DRAFT", action: "REJECT", summary: "Returned to draft" },
  ],
  PUBLISHED: [{ to: "ARCHIVED", action: "ARCHIVE", summary: "Archived" }],
  ARCHIVED: [],
};
...
export async function POST(
  ...
  const publishedAt = target === "PUBLISHED" ? new Date() : procedure.publishedAt;
